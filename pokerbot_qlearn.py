...
def runNN(data,table,randBase=0.1,numEpochs=15,numDenseLayers=1,nDN=1,dropP=0.5,lr=1e-4):
    n = len(data)
    ...
    # 4 possible actions encoded by 1-hot:
    # 0: Fold, 1: Call, 2: Raise by Pot, 3: All-in
    W_fcr = weight_variable([nDN, 4])
    b_fcr = bias_variable([nDN])
    y_result = tf.add(tf.matmul(h_fc_drop,W_fcr),b_fcr),1
    y_predict = tf.argmax(y_result)

    y_ = tf.placeholder(tf.float32, shape=[None,4])
    loss=tf.reduce_sum(tf.square(tf.subtract(y_, y_result)))
    train_step = tf.train.AdamOptimizer(lr).minimize(loss)
    correct_prediction = tf.equal(y_predict, y_)
    ...
    for i in range(numEpochs):
        gameDone = False
        table.resetTable()
        table.preFlop(dealerIndex,log=True)
        self.flop(dealerIndex,log=True)
        self.turn(dealerIndex,log=True)
        winplayers = self.river(dealerIndex,log=True)
        if log:
        ...
        while not gameDone:
            ...
            action,allQ = sess.run([y_predict,y_result],feed_dict={x:xbatch,keep_prob: 1})
            ...
            # TODO: simulate game, get reward from FFnet
            nextState,r,gameDone,_ = env.step(action)
            
            #Obtain the Q' values by feeding the new state through our network
            nextQs = y_result.eval(feed_dict={x:nextState, keep_prob: 1})
            
            #Obtain maxQ' and set our target value for chosen action.
            maxNextQ = np.max(nextQs)
            allQ[action] += r + qlr*maxNextQ
            
            #Train our network using target and predicted Q values
            train_step.run(feed_dict={x:xbatch,y_:allQ, keep_prob: dropP})
            if gameDone:
                #Reduce chance of random action as we train the model.
                if i % 1000 == 0:
                    randBase = 1.0/((i/1000) + 10)
                break
        # TODO
        """    
        if i% printIter == 0:
...
def generateTrainingData(numSamples, fname, iterPrint = 10000,numPlayers=2):
    """
...
class Player():
    ...
    def dictifyFeats(self,currBet,callBet,potBet,allInBet,dealerIndex,playerIndex,tableCards,pot,                bets,remainingPlayers,rd_num,evaluator,log=False,onlyCall=False):
        actDict = {}
        if log:
            actDict["currBet"] = currBet
            actDict["callBet"] = callBet
            actDict["potBet"] = potBet
            actDict["allInBet"] = allInBet
            ...
            actDict["handCards"] = unBitHandCards
            actDict["tableCards"] = unBitTableCards
            actDict["pot"] = pot
            actDict["bets"] = bets
    ...
    def nnAct(self,dealerIndex,playerIndex,tableCards,pot,                bets,remainingPlayers,rd_num,evaluator):
        """
        Responses:
            0: Fold
            1: Call
        ...
        """
        if self.nn == None:
            print("ERROR, no NN found for player " + str(playerIndex))
            return -1
        ...
        potBet = callBet+pot
        allInBet = STACK-currBet
        if callBet > allInBet:
            callBet = allInBet
        if potBet > STACK:
            potBet = allInBet
        actDict = self.dictifyFeats(currBet,callBet,potBet,allInBet,dealerIndex,playerIndex,tableCards,pot,                bets,remainingPlayers,rd_num,evaluator,log=True)
        
        response = nn.runNN(actDict)
        actDict["action"] = response
        if response == 0:
            return (0,0,actDict)
        elif response == 1:
            return (1,callBet,actDict)
        elif response == 2:
            return (2,potBet,actDict)
        elif response == 3:
            return (3,allInBet,actDict)
        
    
    def dumbAct(self,dealerIndex,playerIndex,tableCards,pot,                bets,remainingPlayers,rd_num,evaluator,log=False,onlyCall=False):
        """
        Responses:
            0: Fold
            1: Call
        ...
        callBet = maxBet-currBet
        potBet = callBet+pot
        actDict = self.dictifyFeats(currBet,callBet,potBet,allInBet,dealerIndex,playerIndex,tableCards,pot,                bets,remainingPlayers,rd_num,evaluator,log=log)
        if len(tableCards + self.hand) >= 5:
            playerScore = evaluator.evaluate(self.hand,tableCards)
            handRank = evaluator.get_rank_class(playerScore)
        ...
        for betround in range(2):
            for i in range(self.numPlayers):
                ...
                if self.players[i].nn == None:
                    ...
                    response,bet,actDict = self.players[i].nnAct(
                        dealerIndex,i,self.tableCards,self.pot,
                        self.bets,self.remainingPlayers,rd_num,self.evaluator)
                if log:
                    self.logs.append(actDict)
                    betLogs.append(actDict)
